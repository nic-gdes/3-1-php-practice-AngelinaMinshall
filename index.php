<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>

<header>
        <a href="index.php">
            <img src="pufferfish.png" alt="Pufferfish">
        </a>
        <h1>Homepage</h1>
    </header>
    
    <main>
        <h1>My Favorite Pufferfish</h1>
        <p>Pufferfish are fascinating creatures known for their unique appearance and behavior. They have the ability to inflate themselves when threatened, making them both cute and a bit quirky.</p>

        <div class="grid-container">
            <!-- Example grid items (replace with your own images and titles) -->
            <div class="grid-item">
                <img src="dwarf.jpg" alt="Dwarf Pufferfish">
                <h3>Dwarf Pufferfish</h3>
            </div>
            <div class="grid-item">
                <img src="fahaka.jpg" alt="Fahaka Pufferfish">
                <h3>Fahaka Pufferfish</h3>
            </div>
            <div class="grid-item">
                <img src="blackspotted.jpg" alt="Black Spotted Pufferfish">
                <h3>Black Spotted Pufferfish</h3>
            </div>
            <div class="grid-item">
                <img src="tetraodon.jpg" alt=" Tetraodon Pufferfish">
                <h3>Tetraodon Pufferfish</h3>
            </div>
            <div class="grid-item">
                <img src="toby.jpg" alt="Black saddled toby Pufferfish"> 
                <h3>Black saddled toby Pufferfish</h3>
            </div>
            <div class="grid-item">
                <img src="northern.jpg" alt="Northern Pufferfish">
                <h3>Northern Pufferfish</h3>
            </div>
            <!-- Add more grid items as needed -->
        </div>
    </main>






    <footer>
        <nav>
            <a href="index.php">Home</a>
            <a href="contact.php">Contact</a>
        </nav>
        <p>&copy; <?php echo date("Y"); ?> Pufferfish Appreciation. All Rights Reserved.</p>
    </footer>

</body>
</html>